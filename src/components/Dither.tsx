...
class RetroEffectImpl extends Effect {
  ...
  constructor() {
    ...
    super('RetroEffect', ditherFragmentShader, { uniforms: u });
    this._uniforms = u;
  }
  set colorNum(v: number) { this._uniforms.get('colorNum')!.value = v; }
  get colorNum(): number { return this._uniforms.get('colorNum')!.value; }
  set pixelSize(v: number) { this._uniforms.get('pixelSize')!.value = v; }
  get pixelSize(): number { return this._uniforms.get('pixelSize')!.value; }
}
...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum?: number; pixelSize?: number }>((props, ref) => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return <WrappedRetro ref={ref as any} {...(props as any)} />;
});
...
export default function Dither({
  ...
  return (
    <div style={{ width: '100%', height: '100%' }}>
      <Canvas orthographic camera={{ zoom: 1, position: [0, 0, 100] }} gl={{ antialias: false }}>
        <DitheredWaves
          waveSpeed={waveSpeed}
